...
func getImageClientConnection(imageEndpoint string) (*grpc.ClientConn, error) {
	...
	klog.V(2).Infof("Image connect using endpoints: %v. You should set the endpoint instead.", imageEndpoints)
...
func GetCRIRuntimeService(runtimeEndpoint string) (criapis.RuntimeService, error) {
	...
	for _, endpoint := range runtimeEndpoints {
		...
		if err != nil {
			klog.Warningf("Failed to connect to remote runtime service %s: %v", endpoint, err)
			continue
		}
